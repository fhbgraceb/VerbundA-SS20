...
public class Ersetzen {
	...
	public static void main(String[] args) {
		...
		final int zeilen;
		final int spalten;
		int[][] m;		// Die Matrix kann erst erzeugt werden, wenn die Größe eingelesen wurde
		int[] z = new int[9];
		int nrZufallszahl;
		...
		// Das befüllen der Matrix und die Ausgabe wird in einem Schritt durch geführt
		...
		// Weil der nächste Vorgang komplizierter ist, wird die Ausgabe getrennt durchgeführt
		for (int i = 0; i < m.length; i++) {
			for (int j = 0; j < m[i].length; j++) {
				nrZufallszahl = m[i][j];
